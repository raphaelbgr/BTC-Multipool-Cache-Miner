...
TEST(GBTRegtest, SmokeFetchTemplateAndBuildBlockHex) {
  ...
  // Fetch getblocktemplate with segwit rule and request coinbasetxn capability
  nlohmann::json params = nlohmann::json::array();
  nlohmann::json p = nlohmann::json::object();
  p["rules"] = nlohmann::json::array({"segwit"});
  p["capabilities"] = nlohmann::json::array({"coinbasetxn"});
  params.push_back(p);
  auto tmpl = rpc.call("getblocktemplate", params);
  ASSERT_TRUE(tmpl.has_value()) << "gbt failed: " << rpc.lastError();

  // Update submitter template; coinbase may be real or synthesized
  ...
  // Do not require hasCoinbase() here; allow synthesis if node omits coinbasetxn/default_witness
